...
function index($aCMDResult, $aDSNInfo, $iInstances)
{
    ...
    if (!$aCMDResult['index-noanalyse']) pgsqlRunScript($aCMDResult, 'ANALYSE');
...
function createSearchIndices($aCMDResult)
{
    ...
    pgsqlRunScript($aCMDResult, $sTemplate);
}

function createCountryNames($aCMDResult)
{
    info('Create search index for default country names');

    pgsqlRunScript($aCMDResult, "select getorcreate_country(make_standard_name('uk'), 'gb')");
    pgsqlRunScript($aCMDResult, "select getorcreate_country(make_standard_name('united states'), 'us')");
    pgsqlRunScript($aCMDResult, 'select count(*) from (select getorcreate_country(make_standard_name(country_code), country_code) from country_name where country_code is not null) as x');
    pgsqlRunScript($aCMDResult, "select count(*) from (select getorcreate_country(make_standard_name(name->'name'), country_code) from country_name where name ? 'name') as x");
    ...
    $sSQL .= ') v';
    pgsqlRunScript($aCMDResult, $sSQL);
}
...
function pgsqlRunScriptFile($aCMDResult, $sFilename)
{
...
function pgsqlRunScript($aCMDResult, $sScript, $bfatal = true)
{
    runSQLScript(
        $sScript,
        $bfatal,
...
function pgsqlRunPartitionScript($aCMDResult, $sTemplate)
{
    global $aCMDResult;
    $oDB =& getDB();
...
function replaceTablespace($sTemplate, $sTablespace, $sSql)
{
    if ($sTablespace) {
...
function createSqlFunctions($aCMDResult)
{
    $sModulePath = CONST_Database_Module_Path;
